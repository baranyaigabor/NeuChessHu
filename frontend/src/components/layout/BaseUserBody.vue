    ...
    <div v-if="profileUser" class="mx-auto flex w-full max-w-[1680px] flex-col gap-4 px-3 sm:px-4 md:gap-6 2xl:flex-row 2xl:items-stretch">
        <div class="w-full 2xl:w-[34%] 2xl:flex">
            <div class="profile-eq-card bg-[var(--SideBarBrush)] border !border-black w-full p-4 rounded shadow flex items-center justify-center">
                <PersonalStat :white-matches="profileUser.white_matches" :black-matches="profileUser.black_matches" :myId="userStore.user.data.id"
                />
            </div>
        </div>
        ...
        <div class="w-full 2xl:w-[66%] flex flex-col gap-4">
            <div class="profile-eq-card bg-[var(--SideBarBrush)] border !border-black rounded shadow flex flex-col items-center gap-2">
                <Infos :user="profileUser" :user-id="userStore.user.data.id" :is-owner="isOwner" @save="handleSave"/>
            </div>
